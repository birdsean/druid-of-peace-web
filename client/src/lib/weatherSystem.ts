...
class WeatherManager {
  ...
  checkForWeatherTrigger(currentTurn: number): boolean {
    ...
    if (turnsSinceLastCheck >= minTurns) {
      const triggerChance = Math.min(0.3, (turnsSinceLastCheck - minTurns) / (maxTurns - minTurns) * 0.3);
  ...
  triggerRandomWeather(currentTurn: number): void {
    ...
    const totalWeight = weatherEffects.reduce((sum, effect) => sum + effect.trigger_chance, 0);
    ...
    if (!selectedEffect) {
      selectedEffect = weatherEffects[Math.floor(Math.random() * weatherEffects.length)];
    }

    const duration = Math.floor(Math.random() * (selectedEffect.max_duration - selectedEffect.min_duration + 1)) + selectedEffect.min_duration;
  ...
  triggerSpecificWeather(weatherId: string, currentTurn: number): boolean {
    ...
    const duration = Math.floor(Math.random() * (effect.max_duration - effect.min_duration + 1)) + effect.min_duration;
  ...
  debugGetWeatherHistory(): Array<{ weatherId: string; startTurn: number; endTurn: number }> {
...
export function useWeatherState() {
  const [weatherState, setWeatherState] = React.useState<WeatherState>(globalWeatherManager.getWeatherState());
